...
## コーポレートサイト系ページ(/ndb-renewal など)の方針

`/ndb-renewal` は公開情報を参考にした**研修用モック**であり、本番サイトではない。会社情報を扱うため、研修進行ルールに加えて以下を守る。

- **架空情報を足さない**: 数字・実績・取引先・制度・連絡先を創作しない。未確認情報は載せず、サンプルには「サンプル」と明記する
- **意味を変えない**: 既存サイトの HTML/CSS/JS をコピーせず趣旨を言い換える。会社情報の意味を変える文言変更や、見た目だけの作り替えをしない
- **モック明記を保つ**: フッター等の「研修用モック」注記を消さない
- **UI レビュー観点**: セマンティック HTML / 見出し階層(h1 は1つ) / リンクとボタンの使い分け(`href="#"` のダミー禁止) / フォーカス可視(`:focus-visible`) / カラーコントラスト / スマホ幅。色・グラデーションは CSS 変数で一元管理する
- **レビューの進め方**: 指摘は High / Medium / Low に分けて提示し、許可された範囲だけ最小変更で直す
- **テスト**: ユーザーから見える文言と導線を中心に検証し、文言を変えたら期待値も更新する
- **可読性**: セクション単位で小さく分割し、表示データは配列に分離。共有 CSS/トークンは波及範囲をコメントする
