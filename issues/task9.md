...
| ツール | 特徴 | 速度 |
|-------|------|------|
| **Vitest** | Viteベース、最新、Next.js 16と相性良い | ⚡⚡⚡ |
| Jest | 最も広く使われている | 🐢 |
| Mocha | 柔軟だが設定が複雑 | 🐢 |
...
```bash
cd example1
npm install --save-dev vitest @vitest/ui @vitest/coverage-v8 @vitejs/plugin-react happy-dom @types/node
```

> ℹ️ React コンポーネントを Vitest で扱う場合は `@vitejs/plugin-react` も必要です。カバレッジ計測には `@vitest/coverage-v8` を一緒に入れておくと、後の手順がスムーズです。
...
ステップ1で既に `@vitest/coverage-v8` をインストール済みなら、このステップはスキップして構いません。未導入の場合は以下を実行します：
